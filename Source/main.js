...
function main() {
    ...
    var imageEyeball = ImageHelper.buildImageFromStrings("Eyeball", 1, // scaleMultiplier
    [
        "k", "b", "w", "w", "w", "w", "w", "w", "w", "w"
    ]);
    var materialEyeball = new Material("Eyeball", Color.Instances().White, 1, // diffuse
    1, // specular
    .2, // shininess
    10, // diffuse
    new Texture("Eyeball", imageEyeball));
    var materials = [
        materialEyeball,
        materialRTBang,
        Material.Instances().Green,
    ];
    var sphereEyeball = new Sphere("Eyeball", materialEyeball.name, 100, // radius
    ...
    Globals.Instance.initialize(displaySize, scene);
}
