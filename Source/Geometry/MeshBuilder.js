"use strict";
class MeshBuilder {
    static boxFromNameMaterialAndSize(name, material, size) {
        var meshCube = MeshBuilder.cubeUnitFromNameAndMaterial(name, material);
        var sizeHalf = size.clone().half();
        var transformScaleAndRaiseBottomToGroundLevel = TransformMultiple.fromChildren([
            TransformScale.fromScaleFactors(sizeHalf),
            TransformTranslate.fromOffset(Coords.create().zSet(0 - sizeHalf.z)),
        ]);
        var meshBox = meshCube
            .transformApply(transformScaleAndRaiseBottomToGroundLevel);
        return meshBox;
    }
    static cubeUnitFromNameAndMaterial(name, material) {
        var c3 = (x, y, z) => Coords.fromXYZ(x, y, z);
        var vertexPositions = [
            c3(-1, -1, -1), // 0
            c3(1, -1, -1), // 1
            c3(1, 1, -1), // 2
    ...
    static rectangleFromNameSizeAndMaterial(name, size, material) {
        ...
        var vertices = vertexPositions.map(x => Vertex.fromPos(x));
        var face = Face.fromMaterialNameAndVertexIndices(material.name, [3, 2, 1, 0]);
        return new Mesh(name, vertices, [face]);
    }
}
