...
class Display {
    ...
    drawScene(scene) {
        ...
        for (var y = 0; y < sizeInTiles.y; y++) {
            ...
            for (var x = 0; x < sizeInTiles.x; x++) {
                ...
                this.drawScene_PixelsGetAndDrawForBounds(scene, tileBounds);
            }
        }
    }
    ...
    drawScene_PixelsGetAndDrawForBounds(scene, bounds) {
        ...
        for (var y = boundsMin.y; y < boundsMax.y; y++) {
            ...
            for (var x = boundsMin.x; x < boundsMax.x; x++) {
                pixelPos.x = x;
                var collisionForPixel = this.drawScene_ColorSetFromPixelAtPos(scene, pixelColor, pixelPos);
    ...
    drawScene_ColorSetFromPixelAtPos(scene, surfaceColor, pixelPos) {
        var collisionClosest = this.drawScene_Pixel_FindClosestCollision(scene, pixelPos);
        if (collisionClosest != null) {
            ...
            collidable.surfaceMaterialColorAndNormalForCollision(scene, collisionClosest, surfaceMaterial, surfaceColor, surfaceNormal);
            var intensityFromLightsAll = 0;
            var lights = scene.lighting.lights;
            for (var i = 0; i < lights.length; i++) {
                var light = lights[i];
                var intensity = light.intensityForCollisionMaterialNormalAndCamera(collisionClosest, surfaceMaterial, surfaceNormal, scene.camera);
    ...
    drawScene_Pixel_FindClosestCollision(scene, pixelPos) {
        ...
        displacementFromEyeToPixel.overwriteWith(cameraForward.overwriteWith(cameraOrientation.forward).multiplyScalar(camera.focalLength)).add(cameraRight.overwriteWith(cameraOrientation.right).multiplyScalar(pixelPos.x - this.sizeInPixelsHalf.x)).add(cameraDown.overwriteWith(cameraOrientation.down).multiplyScalar(pixelPos.y - this.sizeInPixelsHalf.y));
