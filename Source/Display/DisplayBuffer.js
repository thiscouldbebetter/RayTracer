...
class DisplayBuffer {
    ...
    pixelAtPosSetToColor(pixelPos, pixelColor) {
        var colorToWriteAsComponentsRgb = pixelColor.componentsRGBA;
        var componentsPerColor = 3;
        var componentFirstIndex = pixelPos.y * componentsPerColor + pixelPos.x;
        for (var i = 0; i < componentsPerColor; i++) {
            var componentToWrite = colorToWriteAsComponentsRgb[i];
            var componentIndex = componentFirstIndex + i;
            this.pixelsAsComponentsRgb[componentIndex] =
                componentToWrite;
        }
    }
    pixelAtPosWriteToColor(pixelPos, colorToOverwrite) {
        var componentsPerColor = 3;
        var componentFirstIndex = pixelPos.y * componentsPerColor + pixelPos.x;
        for (var i = 0; i < componentsPerColor; i++) {
            var componentIndex = componentFirstIndex + i;
            var componentRead = this.pixelsAsComponentsRgb[componentIndex];
